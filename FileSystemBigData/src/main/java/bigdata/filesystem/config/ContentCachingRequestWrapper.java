...
public class ContentCachingRequestWrapper extends HttpServletRequestWrapper {
    ...
    public ServletInputStream getInputStream() throws IOException {
        if (this.inputStream == null || inputStream.isFinished()) {
            ServletInputStream inputStream = getRequest().getInputStream();
            if (inputStream.isFinished()) {
                this.inputStream = new ContentCachingInputStream(this.cachedContent.toByteArray());
            } else {
    ...
    private class ContentCachingInputStream extends ServletInputStream {
        ...
        private final ByteArrayInputStream is2;
        ...
        public ContentCachingInputStream(byte[] buffer) {
            this.is2 = new ByteArrayInputStream(buffer);
            this.is = null;
        }
        ...
        public int readLine(final byte[] b, final int off, final int len) throws IOException {
            ...
            int count = readLine(is, b, off, len);
            writeToCache(b, off, count);
            return count;
        }
