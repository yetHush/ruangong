layui.define('jquery', function (exports) {
    ...
    CheckBox.prototype.checkbox = function (elem, children) {
        ...
        layui.each(nodes, function (index, item) {
            var li = $(['<li class="block' + (item.on ? ' on' : '') + '" value="' + item.name + '" onmouseover="layui.layer.tips(\'' + (options.showType?item.type:item.name) + '\',this,{tips:2})" onmouseout="layui.layer.closeAll(\'tips\');">' + item.name,
                '<i class="choice"><i class="triangle"></i><i class="right layui-icon layui-icon-ok"></i></i>' +
                (options.showDel ? '<i class="del"><i class="layui-icon layui-icon-delete"></i>' : '') +
                '</i><span class="hide">' + (item.on ? '<input type="hidden" name="' + item.name + '" value="' + item.type + '">' : '') + '</span></li>'].join(''));
            elem.append(li);
            //触发点击节点回调
...
}).link(layui.cache.base + 'checkbox/checkbox.css', 'checkboxcss');
